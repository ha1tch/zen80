...
func (cpu *Z80) executeBlock0(opcode uint8, y, z, p, q uint8) int {
	...
	case 2:
		...
		switch y {
		case 0: // LD (BC),A
			cpu.memWrite(cpu.BC(), cpu.A)
			cpu.WZ = (uint16(cpu.A) << 8) | ((cpu.BC() + 1) & 0xFF)
			return 7
		case 1: // LD A,(BC)
			cpu.A = cpu.memRead(cpu.BC())
			cpu.WZ = cpu.BC() + 1
			return 7
		case 2: // LD (DE),A
			cpu.memWrite(cpu.DE(), cpu.A)
			cpu.WZ = (uint16(cpu.A) << 8) | ((cpu.DE() + 1) & 0xFF)
			return 7
		case 3: // LD A,(DE)
			cpu.A = cpu.memRead(cpu.DE())
			cpu.WZ = cpu.DE() + 1
			return 7
		case 4: // LD (nn),HL
			addr := cpu.fetchWord()
	...
	case 4: // INC r
		if y == 6 { // (HL)
			...
			cpu.memWrite(addr, cpu.inc8(val))
			return 11
		}
	...
	case 5: // DEC r
		if y == 6 { // (HL)
			addr := cpu.HL()
			val := cpu.memRead(addr)
			cpu.memWrite(addr, cpu.dec8(val))
			return 11
		}
		reg := cpu.getRegister8(y)
		*reg = cpu.dec8(*reg)
...
func (cpu *Z80) executeBlock2(opcode uint8, y, z uint8) int {
	...
	switch y {
	case 0:
		cpu.add8(val) // ADD A,r
	case 1:
		cpu.adc8(val) // ADC A,r
	case 2:
		cpu.sub8(val) // SUB r
	case 3:
		cpu.sbc8(val) // SBC A,r
	case 4:
		cpu.and8(val) // AND r
	case 5:
		cpu.xor8(val) // XOR r
